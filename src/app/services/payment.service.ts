...
export class PaymentService {
  ...
  findAllPaymentsPaid(): Observable<IPayment[]> {
    return this.http.get<IPayment[]>(`${this.PAYMENT_URL}paid`);
  }
  ...
  searchByNumber(payment_number: number): Observable<IPayment> {
    return this.http.get<IPayment>(
      `${this.PAYMENT_URL}search/${payment_number}`
    );
  }

  getLastNumber(): Observable<number> {
    return this.http.get<number>(
      'http://localhost:3000/payment/get-last-number/:'
    );
  }
}
